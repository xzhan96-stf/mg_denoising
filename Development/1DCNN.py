...
Dir_modeling_results = 'H:\\My Drive\\Paper\\MG Denoising\\Prediction'
Dir_models = 'H:\\My Drive\\Paper\\MG Denoising\\Model'
...
def compute_validation_loss(valloader,model,loss_function):
    for i, data in enumerate(valloader, 0):
        ...
        output = model(input)
    return loss_function(output, target).detach()
...
if __name__ == '__main__':
    ...
    trainloader = torch.utils.data.DataLoader(train_dataset,batch_size=2400,shuffle=True, num_workers=1)
    ...
    MAL,RMSL,R2L,y_train,y_train_pred,_ = evaluate(trainloader,convautoencoder)
    ...
    specs = 'ConvAutoencoder_fullset_symmetry_' + target + '_channels_' + layers + '_kernel_size_' + str(kernel_size) + '_initial lr_' + str(init_learning_rate) + '_epoch_' + str(epochs) + 'batchnorm_l2_1e-2'
